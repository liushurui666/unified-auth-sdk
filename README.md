...
Auth Service / 内嵌 Hosted Auth
  - 持有 OAuth 配置
  - 持有用户、会话和账号数据
  - 根据当前业务项目的 clientId 渲染 Hosted Login
...
业务项目使用内嵌 Hosted Auth 时，登录页 UI 由 `@rc-tool/unified-auth-hosted-service` 的 `loginPageComponent` 控制。不传组件时会使用 SDK 默认组件和预设样式；传组件时只定制背景图、Logo、文案和 provider 展示，OAuth、state、callback、session 仍然全部由 SDK 内部处理。
...
业务侧不需要传多应用数组。内嵌模式就是“一个业务项目挂一套自己的认证路由”，所以直接在 `createHostedAuthRouteHandlers` 上声明当前项目的 `clientId`、展示名称和回跳地址即可。旧的 `loginPage` 和 `appearance.backgroundImageUrl` 仍然兼容，但新项目推荐使用 `loginPageComponent`。
